...
const runSample = (columnDetailUid: string, columnSampleUid: string, model: string, splitPerTokens: number) => request.post('/sample_batches', { columnDetailUid, columnSampleUid, model, splitPerTokens, });

const run = (columnSampleUid: string) => request.post(`/sample_batches/${columnSampleUid}/run`);

const requestFullCapacity = (uid: string, model: string, splitPerTokens: number) => request.post(`/batches`, { uid, model, splitPerTokens, })
